...
rule demux_dual:
    input:
        ...
        r2 = lambda wildcards: glob.glob("{directory}/{platform_run}_S?_L001_R2_001.fastq.gz".format(directory=config["read_directory"],
            platform_run=config['samples'][wildcards.sample])),
        fwd_barcorde="{run}/demux/{sample}_0.fasta",
        rev_barcorde= "{run}/demux/{sample}_1.fasta"
    output:
    ...
    shell:
        """
        cutadapt -j {threads} --revcomp -e 1 --no-indels -g file:{input.fwd_barcorde} -G file:{input.rev_barcorde} -o '{wildcards.run}/demux/demultiplexed-{{name}}_R1.fastq.gz' -p '{wildcards.run}/demux/demultiplexed-{{name}}_R2.fastq.gz' {input.r1} {input.r2}
        """
# rule clumpify:
#     input:
