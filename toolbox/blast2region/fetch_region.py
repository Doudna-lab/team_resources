...
# Load config file
with open("/Users/bellieny/projects/team_resources/toolbox/blast2region/config/blast_config.yaml", "r") as f:
...
def gb_plier(uid_to_gb, uid_to_acc, win_size):
	gbk_target = {}
	prot_dict = {}
	for hit_uid in uid_to_gb:
		gbk = uid_to_gb[hit_uid]
...
def main():
	...
	# DEBUG
	df = pd.read_csv("/Users/bellieny/projects/team_resources/toolbox/blast2region/scratch/rubisco_idlist.csv")
	df = df[(df['Annotation'] == "Bona_Fide") & (df['subForm'] != "B")].head()
	hit_list = df['NCBI_Accession_ID'].head().tolist()
	...
	nuc_uid_list, hit_to_link, hits_not_found = ptn_to_nuc(hit_list, "nuccore")
	...
	targeg_gb_dict, target_hit_dict = gb_plier(gb_seqrec, hit_to_link, 0)
